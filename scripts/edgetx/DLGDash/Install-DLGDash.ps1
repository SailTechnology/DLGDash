...
if ($board -notin @('pa01', 'v16', 'v12', 'x9d', 'x9d+', 'x9d+2019', 'gx12', 'zorro', 't14')) { throw "Unsupported radio board: $board" }
...
foreach ($name in $sources) {
    ...
    $compiled = [IO.Path]::ChangeExtension($destination, '.luac')
    if ([IO.Path]::GetExtension($name) -eq '.lua' -and (Test-Path -LiteralPath $compiled)) {
