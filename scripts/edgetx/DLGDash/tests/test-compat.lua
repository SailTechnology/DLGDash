...
for _, minor in ipairs({ 7, 8, 9, 10, 11 }) do
  ...
  eq(A.legacy, minor ~= 11); eq(A.mixedOutputs, minor ~= 11)
  eq(A.toneVolume, minor >= 11)
  eq(A.output(0), minor == 11 and -256 or 512)
  ...
  value, valid = A.value(304)
  eq(valid, minor ~= 11) -- Legacy cannot expose individual sensor expiry.
end
Bitmap, lcd = {}, {}
