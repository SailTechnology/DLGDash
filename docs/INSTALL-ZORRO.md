...
1. 确认电脑备份完整后，在遥控器内容盘把 `WIDGETS/DLGDash` 改名为 `DLGDash-before-beta5`。如果此名字已存在，换一个没有被使用的备份名字，不要覆盖。
...
3. 更新用户：把刚改名的 `DLGDash-before-beta5/profiles` 文件夹复制进新的 `WIDGETS/DLGDash`，合并以保留各模型的插件设置。只复制 profiles，不把旧 `.lua`、`.luac` 或整个旧目录复制回来。
